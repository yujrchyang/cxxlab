...
BitmapAllocator::BitmapAllocator(int64_t capacity, int64_t alloc_unit,
    ...
    : Allocator(name, capacity, alloc_unit),
      impl_(std::make_unique<AllocatorLevel02>()) {
    impl_->_init(capacity, alloc_unit, false);
}
...
int64_t BitmapAllocator::allocate(uint64_t want, uint64_t unit,
    ...
    cxxlab_assert(unit >= impl_->l1.l0_granularity);
    uint64_t allocated = 0;
    int r = impl_->_allocate_l2(want, unit, max_alloc_size, hint,
                                &allocated, extents);
...
uint64_t BitmapAllocator::get_free() {
    std::lock_guard l(impl_->lock);
    return impl_->available;
}

double BitmapAllocator::get_fragmentation() {
    std::lock_guard l(impl_->lock);
    uint64_t total = impl_->l1.unalloc_l1_count + impl_->l1.partial_l1_count;
    if (total == 0)
        return 0.0;
    return static_cast<double>(impl_->l1.partial_l1_count) / total;
}

void BitmapAllocator::dump() {
    std::map<size_t, size_t> bins;
    impl_->collect_stats(bins);
...
uint64_t BitmapAllocator::claim_free_to_left(uint64_t offset) {
    return impl_->claim_free_to_left(offset);
}

uint64_t BitmapAllocator::claim_free_to_right(uint64_t offset) {
    return impl_->claim_free_to_right(offset);
}
